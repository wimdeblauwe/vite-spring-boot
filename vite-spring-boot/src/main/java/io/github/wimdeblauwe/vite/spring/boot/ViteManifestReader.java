...
public class ViteManifestReader {
  ...
  public ViteManifestReader(JsonMapper jsonMapper,
    ...
    if (configurationProperties.mode() == ViteConfigurationProperties.Mode.BUILD) {
      ...
      if (resource.exists()) {
        try (InputStream inputStream = resource.getInputStream()) {
          MapType type = jsonMapper.getTypeFactory().constructMapType(Map.class, String.class, ManifestEntry.class);
          manifest = jsonMapper.readValue(inputStream, type);
        }
      } else {
  ...
  public record ManifestEntry(String file, String src, Boolean isEntry, List<String> css, List<String> imports) {
    public ManifestEntry(String file, String src, Boolean isEntry, List<String> css, List<String> imports) {
      this.file = file;
      this.src = src;
      this.isEntry = isEntry != null && isEntry;
      this.css = css != null ? css : List.of();
